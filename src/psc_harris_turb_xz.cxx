...

  {
    ...
    for (int64_t n = 0; n < Ne_sheet / n_global_patches; n++) {
      ...
      injector.reweight(psc::particle::Inject{
        {x, y, z}, {ux, uy, uz}, weight_s, KIND_ELECTRON}); //This is injecting electrons in the sheet
      ...
      injector.reweight(
        psc::particle::Inject{{x, y, z}, {ux, uy, uz}, weight_s, KIND_ION}); //This is injecting ions in the sheet
    }
    ...
    for (int64_t n = 0; n < Ne_back / n_global_patches; n++) {
      ...
      double xpos = Rng_uniform(rng, xmin, xmax);
      double ypos = Rng_uniform(rng, ymin, ymax);      
      double zpos = Rng_uniform(rng, zmin, zmax);
            
      // This is to include the Alfven waves into the backgroudn plasma only. Here is where I am. 26/04/2022
      // things to do. See where to include the definitions of dv_alf_x and db_alf_x  
          // Alfven waves parameters
      //----------------------------------------------------------------------------
      // Defining the Alfven wave parameters. //This is the alfven magnetic fluctuation. Jeff
      //--------------------------------------------------------------------------------------
      double b0 = phys.b0;
      double Lx = phys.Lx, Ly = phys.Ly, Lz = phys.Lz;
      
      double sp = 1./3.;         //Spectral index for the alfven wave (1/3 for AW and 2/3 for KAW)
      double p_a = .1;                // fraction of B0 so dB=(1/p)B0
      double crit_fact = 0.1;    //critical balance Isotropization coefficient 10^{-4/3} for 10^{-3}      
      ...
      double B0 = p_a*b0;
      double dbdv = phys.v_A/B0;// This is the factor that has to be defined consistently. FIXME Jeff
    
      double mn_per1 = m_per , mn_per2 = m_per , mn_per3 = m_per , mn_per4 = m_per , mn_per5 = m_per , mn_per6 = m_per , mn_per7 = m_per , mn_per8 = m_per ; // wave mode factor per
      double mn_par1 = m_par , mn_par2 = m_par , mn_par3 = m_par , mn_par4 = m_par , mn_par5 = m_par , mn_par6 = m_par , mn_par7 = m_par , mn_par8 = m_par ; // wave mode factor par

      double Thephase1[4]={0.987*2.*M_PI, 0.666*2.*M_PI, 0.025*2.*M_PI, 0.954*2.*M_PI}; // Random phase of the waves
      double Thephase2[4]={0.781*2.*M_PI, 0.846*2.*M_PI, 0.192*2.*M_PI, 0.778*2.*M_PI};
    
      double phase1 = Thephase1[0], phase2 = Thephase2[0]; 
      double phase3 = Thephase1[1], phase4 = Thephase2[1]; 
      double phase5 = Thephase1[2], phase6 = Thephase2[2]; 
      double phase7 = Thephase1[3], phase8 = Thephase2[3];   
    
      double phi1=0., phi2=M_PI/2., phi3=M_PI, phi4=3.*M_PI/2., phi5=M_PI/4., phi6=3.*M_PI/4., phi7=5.*M_PI/4., phi8=7.*M_PI/4.; // polarizationa angle of the AW
    
      double k_x1= mn_per1*(2. * M_PI /Lx) , k_y1 = mn_per1*(2. * M_PI /Ly) * (1.) , k_z1 = mn_par1*(2. * M_PI /Lz) * (0.);  // wave vector components 
      double k_x2= mn_per2*(2. * M_PI /Lx) , k_y2 = mn_per2*(2. * M_PI /Ly) * (0.) , k_z2 = mn_par2*(2. * M_PI /Lz) * (1.);  
      double k_x3= mn_per3*(2. * M_PI /Lx) , k_y3 = mn_per3*(2. * M_PI /Ly) * (-1.), k_z3 = mn_par3*(2. * M_PI /Lz) * (0.);  
      double k_x4= mn_per4*(2. * M_PI /Lx) , k_y4 = mn_per4*(2. * M_PI /Ly) * (0.) , k_z4 = mn_par4*(2. * M_PI /Lz) * (-1.);      
      double k_x5= mn_per5*(2. * M_PI /Lx) , k_y5 = mn_per5*(2. * M_PI /Ly) * (1.) , k_z5 = mn_par5*(2. * M_PI /Lz) * (1.);  
      double k_x6= mn_per6*(2. * M_PI /Lx) , k_y6 = mn_per6*(2. * M_PI /Ly) * (-1.), k_z6 = mn_par6*(2. * M_PI /Lz) * (1.);  
      double k_x7= mn_per7*(2. * M_PI /Lx) , k_y7 = mn_per7*(2. * M_PI /Ly) * (-1.), k_z7 = mn_par7*(2. * M_PI /Lz) * (-1.);  
      double k_x8= mn_per8*(2. * M_PI /Lx) , k_y8 = mn_per8*(2. * M_PI /Ly) * (1.) , k_z8 = mn_par8*(2. * M_PI /Lz) * (-1.);
    
      double k_a_per1 = sqrt(k_z1*k_z1 + k_y1*k_y1); 
      double k_a_per2 = sqrt(k_z2*k_z2 + k_y2*k_y2);    
      double k_a_per3 = sqrt(k_z3*k_z3 + k_y3*k_y3); 
      double k_a_per4 = sqrt(k_z4*k_z4 + k_y4*k_y4);
      double k_a_per5 = sqrt(k_z5*k_z5 + k_y5*k_y5); 
      double k_a_per6 = sqrt(k_z6*k_z6 + k_y6*k_y6);    
      double k_a_per7 = sqrt(k_z7*k_z7 + k_y7*k_y7); 
      double k_a_per8 = sqrt(k_z8*k_z8 + k_y8*k_y8);
    
      double k_a_par1 = k_x1, k_a_par2 = k_x2, k_a_par3 = k_x3, k_a_par4 = k_x4, k_a_par5 = k_x5, k_a_par6 = k_x6, k_a_par7 = k_x7, k_a_par8 = k_x8;  
    
      double Amp1 = B0 * crit_fact *pow((k_a_per1), -sp) ; //According to the critical balance Amp[i]=B0*crit_fact * K_a_perp[i]^(-sp);    
      double Amp2 = B0 * crit_fact *pow((k_a_per2), -sp) ;
      double Amp3 = B0 * crit_fact *pow((k_a_per3), -sp) ;
      double Amp4 = B0 * crit_fact *pow((k_a_per4), -sp) ;
      double Amp5 = B0 * crit_fact *pow((k_a_per5), -sp) ;
      double Amp6 = B0 * crit_fact *pow((k_a_per6), -sp) ;    
      double Amp7 = B0 * crit_fact *pow((k_a_per7), -sp) ;
      double Amp8 = B0 * crit_fact *pow((k_a_per8), -sp) ;

      double dB_ay1 = -Amp1 * cos ( k_y1* ypos + k_z1 * zpos + k_x1* xpos + phase1 )*sin(phi1) ;
      double dB_ay2 = -Amp2 * cos ( k_y2* ypos + k_z2 * zpos - k_x2* xpos + phase2 )*sin(phi2) ;
      double dB_ay3 = -Amp3 * cos ( k_y3* ypos + k_z3 * zpos + k_x3* xpos + phase3 )*sin(phi3) ;
      double dB_ay4 = -Amp4 * cos ( k_y4* ypos + k_z4 * zpos - k_x4* xpos + phase4 )*sin(phi4) ;
      double dB_ay5 = -Amp5 * cos ( k_y5* ypos + k_z5 * zpos + k_x5* xpos + phase5 )*sin(phi5) ;
      double dB_ay6 = -Amp6 * cos ( k_y6* ypos + k_z6 * zpos - k_x6* xpos + phase6 )*sin(phi6) ;
      double dB_ay7 = -Amp7 * cos ( k_y7* ypos + k_z7 * zpos + k_x7* xpos + phase7 )*sin(phi7) ;
      double dB_ay8 = -Amp8 * cos ( k_y8* ypos + k_z8 * zpos - k_x8* xpos + phase8 )*sin(phi8) ;
    
      double dB_az1 = -dB_ay1;
      double dB_az2 = -dB_ay2;
      double dB_az3 = -dB_ay3;  
      double dB_az4 = -dB_ay4;  
      double dB_az5 = -dB_ay5;  
      double dB_az6 = -dB_ay6;
      double dB_az7 = -dB_ay7;  
      double dB_az8 = -dB_ay8;

      double C1 = B0/sqrt(sqr(Amp1)+sqr(Amp2)+sqr(Amp3)+sqr(Amp4)+sqr(Amp5)+sqr(Amp6)+sqr(Amp7)+sqr(Amp8)); //normalization factor

      //magnetic field
      double dB_ayT = C1*(dB_ay1 + dB_ay2 + dB_ay3 + dB_ay4 + dB_ay5 + dB_ay6 + dB_ay7 + dB_ay8);
      double dB_azT = C1*(dB_az1 + dB_az2 + dB_az3 + dB_az4 + dB_az5 + dB_az6 + dB_az7 + dB_az8);
    
      //velocities
      double dv_ayT = -dbdv*C1*(dB_ay1 + dB_ay2 -dB_ay3 + dB_ay4 -dB_ay5 + dB_ay6 -dB_ay7 + dB_ay8);
      double dv_azT = -dbdv*C1*(dB_az1 + dB_az2 -dB_az3 + dB_az4 -dB_az5 + dB_az6 -dB_az7 + dB_az8);
      //--------------------------------------------------------------------------------------          
       
      //----------------------------------------------------------------------------
      
      //Double3 u{Rng_normal(rng, 0, vtheb), Rng_normal(rng, 0, vtheb),
      //          Rng_normal(rng, 0, vtheb)}; // comment this when defining the velocity uxb ,... 
      double uxbe = Rng_normal(rng, 0, vtheb);
      double uybe = Rng_normal(rng, 0, vtheb);
      double uzbe = Rng_normal(rng, 0, vtheb);
      double ue_total_x = uxbe;
      double ue_total_y = uybe + dv_ayT;
      double ue_total_z = uzbe + dv_azT;
      // instead of u put {u_total_x,u_total_y, u_total_z} // repeat this for the ions
                
      injector.reweight(psc::particle::Inject{{xpos,ypos,zpos}, {ue_total_x,ue_total_y,ue_total_z}, weight_b, KIND_ELECTRON}); //This is injecting electrons in the BACKGROUND

      //u = {Rng_normal(rng, 0, vthib), Rng_normal(rng, 0, vthib),
      //     Rng_normal(rng, 0, vthib)};
      double uxbi = Rng_normal(rng, 0, vthib);
      double uybi = Rng_normal(rng, 0, vthib);
      double uzbi = Rng_normal(rng, 0, vthib);
      double ui_total_x = uxbi ;
      double ui_total_y = uybi + dv_ayT;
      double ui_total_z = uzbi + dv_azT;
      injector.reweight(psc::particle::Inject{{xpos,ypos,zpos}, {ui_total_x,ui_total_y,ui_total_z}, weight_b, KIND_ION}); //This is injecting Ions in the BACKGROUND
    }
  }
...
// ======================================================================
// initializeParticles. // This used the inputs from loading particles. 
...
// ======================================================================
// initializeFields. // This initialize the magnetic field components here

void initializeFields(MfieldsState& mflds)
{
  double b0 = phys.b0, dbx = phys.dbx, dbz = phys.dbz;
  double L = phys.L, Lx = phys.Lx, Ly = phys.Ly, Lz = phys.Lz, Lpert = phys.Lpert;
  double cs = cos(g.theta), sn = sin(g.theta);
  ...
  setupFields(mflds, [&](int m, double crd[3]) {
    double x = crd[0], y=crd[1], z = crd[2];
      ...
      double dB_ay1 = -Amp1 * cos ( k_y1* y + k_z1 * z + k_x1* x + phase1 )*sin(phi1) ;
      double dB_ay2 = -Amp2 * cos ( k_y2* y + k_z2 * z - k_x2* x + phase2 )*sin(phi2) ;
      double dB_ay3 = -Amp3 * cos ( k_y3* y + k_z3 * z + k_x3* x + phase3 )*sin(phi3) ;
      double dB_ay4 = -Amp4 * cos ( k_y4* y + k_z4 * z - k_x4* x + phase4 )*sin(phi4) ;
      double dB_ay5 = -Amp5 * cos ( k_y5* y + k_z5 * z + k_x5* x + phase5 )*sin(phi5) ;
      double dB_ay6 = -Amp6 * cos ( k_y6* y + k_z6 * z - k_x6* x + phase6 )*sin(phi6) ;
      double dB_ay7 = -Amp7 * cos ( k_y7* y + k_z7 * z + k_x7* x + phase7 )*sin(phi7) ;
      double dB_ay8 = -Amp8 * cos ( k_y8* y + k_z8 * z - k_x8* x + phase8 )*sin(phi8) ;
    ...
    switch (m) {
      ...
      case HY:
        return -sn * b0 * tanh(z / L) + b0 * g.bg + dB_azT;

      case HZ:
        return dbz * cos(M_PI * z / Lz) * sin(2.0 * M_PI * (x - 0.5 * Lx) / Lpert) + dB_azT;
