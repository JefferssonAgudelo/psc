...
struct SetupFields
{
  ...
  template <typename FUNC>
  static void run_indices(Mfields& mf, FUNC&& func)
  {
    const auto& grid = mf.grid();
    mpi_printf(grid.comm(), "**** Setting up fields...\n");
...
template <typename MF, typename FUNC>
void setupFieldsIndices(MF& mflds, FUNC&& func)
{
  detail::SetupFields<MF>::run_indices(mflds, std::forward<FUNC>(func));
}
